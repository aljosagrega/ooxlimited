...
class Omero_Elementor_Widget_Service_Accordion extends Omero_Base_Widgets {
    ...
    private function handle_setting($settings) {
        ...
        if (!empty($settings['services_list'])) {
            ...
            foreach ($settings['services_list'] as $item) {
                ...
                if (!empty($item['selected_icon']['id'])) {
                    $icon = wp_get_attachment_image($item['selected_icon']['id'], 'large');
                }

                $side_title .= $this->get_render_side_titles($i, $_id);
                $side_image .= $this->get_render_side_images($i, $_id, $icon);
        ...
        <div <?php $this->print_render_attribute_string('service-wrapper'); ?>>
            <div class="omero-swiper swiper omero-service-side-images-swiper">
                <ul class="omero-service omero-list-wrapper clear-list-style swiper-wrapper">
    ...
    private function get_render_side_images($index, $_id, $icon = '') {
        ob_start();
        $class = 'swiper-slide elementor-repeater-item-'.$_id;
        if ($index === 1) {
            $class .= ' actived';
        }
        ?>
        <li class="<?php echo esc_attr($class); ?>">
            <div class="service-accordion-block-wrapper">
                <div class="service-block omero-path-wrapper only-bottom-left">
                    <img src="/wp-content/uploads/2026/01/Group-6361.png" class="service-block-image-bg"/>
                    <?php if (!empty($icon)) : ?>
                        <div class="service-block-image-front"><?php echo $icon; ?></div>
                    <?php endif; ?>
                </div>
                <?php
                if (!empty($icon)) {
                    printf('<div class="service-icon">%s</div>', $icon);
                }
                ?>
            </div>
        </li>
