...
class Lexus_WP_Widget_Recent_Posts extends WP_Widget_Recent_Posts {
	public function widget( $args, $instance ) {
		if ( ! isset( $args['widget_id'] ) ) {
			$args['widget_id'] = $this->id;
		}

		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : esc_html__( 'Recent Posts', 'themelexus-debug' );
		...
		<ul>
			<?php foreach ( $r->posts as $recent_post ) : ?>
				...
				$title        = ( ! empty( $post_title ) ) ? $post_title : esc_html__( '(no title)', 'themelexus-debug' );
				?>
				<li>
					...
					<div class="recent-posts-info">
                        ...
                        <div class="recent-posts-top-info entry-meta-top">
                            <?php lexus_post_meta(['show_date' => true, 'show_author' => false, 'show_cat' => true,  'show_comment' => false]); ?>
                        </div>
<!--                        --><?php //if ( $show_date ) : ?>
...
add_action('widgets_init', function () {
	register_widget('Lexus_WP_Widget_Recent_Posts');
});
