...
class Omero_Elementor_Blog_Archive_Featured_Grid extends Omero_Base_Widgets {
	...
	/**
	 * Date + byline: child theme team writer if available, else WordPress author.
	 *
	...
	protected function render_meta_line($for_post = null) {
		...
		$writer  = function_exists('omero_child_get_post_team_writer') ? omero_child_get_post_team_writer($current) : null;

		if (apply_filters('omero_baf_console_log_render_meta_line', defined('WP_DEBUG') && WP_DEBUG)) {
			$post_id = ($current instanceof WP_Post) ? (int) $current->ID : 0;
			$debug   = array(
				'label'                       => 'omero-blog-archive: render_meta_line',
				'postId'                      => $post_id,
				'postTitle'                   => ($current instanceof WP_Post) ? $current->post_title : null,
				'helperLoaded'                => function_exists('omero_child_get_post_team_writer'),
				'meta__post_team_writer_id'   => $post_id ? get_post_meta($post_id, '_post_team_writer_id', true) : null,
				'meta_post_team_writer_id'    => $post_id ? get_post_meta($post_id, 'post_team_writer_id', true) : null,
				'writerPostId'                => ($writer instanceof WP_Post) ? (int) $writer->ID : null,
				'writerTitle'                 => ($writer instanceof WP_Post) ? $writer->post_title : null,
				'fallbackToWpAuthor'          => !($writer instanceof WP_Post),
			);
			printf(
