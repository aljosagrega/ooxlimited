(function ($) {
    ...
    const upsellsSwiper = $('.upsells .omero-theme-swiper');
    const relatedSwiper = $('.related .omero-theme-swiper');
    const crossSwiper = $('.cross-sells .omero-theme-swiper');
    if (crossSwiper.length > 0) {
        const ElementSettings = crossSwiper.data('settings');
        new Swiper('.cross-sells .omero-swiper', getSwiperSettings(ElementSettings));
    }
    if (upsellsSwiper.length > 0) {
        const ElementSettings = upsellsSwiper.data('settings');
        new Swiper('.upsells .omero-swiper', getSwiperSettings(ElementSettings));
    }
    if (relatedSwiper.length > 0) {
        const ElementSettings = relatedSwiper.data('settings');
        new Swiper('.related .omero-swiper', getSwiperSettings(ElementSettings));
    }
