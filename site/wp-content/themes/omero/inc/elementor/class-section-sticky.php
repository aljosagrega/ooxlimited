...
class Omero_Elementor_Sticky_Section {
    ...
    public function scripts() {
        ...
        wp_register_script('elementor-sticky', get_template_directory_uri() . '/assets/js/vendor/elementor-sticky.js', ['jquery'], $omero_version, true);
        wp_enqueue_script('omero-elementor-sticky', get_template_directory_uri() . '/assets/js/vendor/sticky.js', ['jquery', 'elementor-frontend', 'elementor-sticky'], $omero_version, true);
    }
    ...
    public function register_controls(Element_Base $element) {
        $element->add_control(
            'sticky_show_up',
            [
                'label'              => esc_html__('Sticky When Up', 'omero'),
        ...
        $element->add_control(
            'sticky',
            [
                'label'              => esc_html__('Sticky', 'omero'),
                'type'               => Controls_Manager::SELECT,
                'options'            => [
                    ''       => esc_html__('None', 'omero'),
                    'top'    => esc_html__('Top', 'omero'),
                    'bottom' => esc_html__('Bottom', 'omero'),
                ],
                'separator'          => 'before',
                'render_type'        => 'none',
                'frontend_available' => true,
        ...
        foreach ($active_devices as $device) {
            $label                          = 'desktop' === $device ? esc_html__('Desktop', 'omero') : $active_breakpoint_instances[$device]->get_label();
        ...
        $element->add_control(
            'sticky_on',
            [
                'label'              => esc_html__('Sticky On', 'omero'),
        ...
        $element->add_responsive_control(
            ...
            [
                'label'              => esc_html__('Effects Offset', 'omero'),
                'type'               => Controls_Manager::NUMBER,
                'default'            => 0,
                'min'                => 0,
                'max'                => 1000,
        ...
        if ($this->is_instance_of($element, $types)) {
            ...
            $element->add_control(
                'sticky_parent',
                [
                    'label'              => esc_html__('Stay In Column', 'omero'),
                    'type'               => Controls_Manager::SWITCHER,
...
new Omero_Elementor_Sticky_Section();
