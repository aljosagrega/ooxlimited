...
class Omero_Elementor_Widget_Product_Add_To_Cart extends Elementor\Widget_Base {

	use Omero_Elementor_Product_Base_Trait;

	public function get_name() {
		return 'omero-product-add-to-cart';
	}

	public function get_title() {
		return esc_html__( 'Omero Product Add To Cart', 'omero' );
	}
	...
	private function is_loop_item() {
		return 'loop-item' === Plugin::$instance->documents->get_current()->get_type();
	}

	private function is_loop_item_template_edit() {
		return ( Plugin::$instance->editor->is_edit_mode() && $this->is_loop_item() );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					'' => esc_html__( 'Inline', 'omero' ),
					'stacked' => esc_html__( 'Stacked', 'omero' ),
					'auto' => esc_html__( 'Auto', 'omero' ),
				],
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'size_units' => [ 'px', '%', 'em', 'rem', 'vw', 'custom' ],
                'selectors' => [
                    '{{WRAPPER}} .cart button, {{WRAPPER}} .cart .button' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
            ]
        );
		...
		$this->add_control(
			'button_transition',
			[
				'label' => esc_html__( 'Transition Duration', 'omero' ) . ' (s)',
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'View Cart', 'omero' ),
		...
		$this->start_controls_section(
			...
			[
				'label' => esc_html__( 'Quantity', 'omero' ),
				'tab' => Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->start_controls_section(
			...
			[
				'label' => esc_html__( 'Variations', 'omero' ),
				'tab' => Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Space Between', 'omero' ),
				'type' => Controls_Manager::SLIDER,
				'size_units' => [ 'px', 'em', 'rem', 'custom' ],
				'range' => [
					'px' => [
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Select field', 'omero' ),
				'type' => Controls_Manager::HEADING,
				'separator' => 'before',
			]
		);
...
$widgets_manager->register(new Omero_Elementor_Widget_Product_Add_To_Cart());