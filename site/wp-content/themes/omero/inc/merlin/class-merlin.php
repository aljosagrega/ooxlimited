...
//		add_action( 'after_switch_theme', array( $this, 'switch_theme' ) );
    ...
    public function admin_page() {
        ...
        $texts = array(
            'something_went_wrong' => esc_html__('Something went wrong. Please refresh the page and try again!', 'omero'),
        );
...
<!--            --><?php //do_action( 'admin_head' ); ?>
    ...
    public function svg($args = array()) {
        ...
        if (empty($args)) {
            return esc_html__('Please define default parameters in the form of an array.', 'omero');
        }

        // Define an icon.
        if (false === array_key_exists('icon', $args)) {
            return esc_html__('Please define an SVG icon filename.', 'omero');
        }
    ...
    public function steps() {
        ...
        if (!empty($this->import_files)) {
            $this->steps['content'] = array(
                'name' => esc_html__('Content', 'omero'),
                'view' => array($this, 'content'),
            );
        }

        $this->steps['ready'] = array(
            'name' => esc_html__('Ready', 'omero'),
    ...
    protected function welcome() {
        ...
        <?php
        $this->logger->debug(esc_html__('The welcome step has been displayed', 'omero'));
    }
    ...
    protected function license() {
        ...
        <?php
        $this->logger->debug(esc_html__('The license activation step has been displayed', 'omero'));
    }
    ...
    protected function child() {
        ...
        <?php
        $this->logger->debug(esc_html__('The child theme installation step has been displayed', 'omero'));
    }
    ...
    protected function plugins() {
                        ...
                        <?php foreach ($required_plugins as $slug => $plugin) : ?>
                            <li data-slug="<?php echo esc_attr($slug); ?>">
                                ...
                                <label for="default_plugins_<?php echo esc_attr($slug); ?>">
                                    ...
                                    <span class="badge">
									<span class="hint--top" aria-label="<?php esc_attr_e('Required', 'omero'); ?>">
										<?php esc_html_e('req', 'omero'); ?>
									</span>
								</span>
                                </label>
                            </li>
        ...
        <?php
        $this->logger->debug(esc_html__('The plugin installation step has been displayed', 'omero'));
    }
    ...
    protected function content() {
            ...
            <?php if (1 < count($this->import_files)) : ?>
                ...
                <div class="merlin__select-control-wrapper">
                    ...
                    <div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__('Select Demo', 'omero'); ?>">
        ...
        <?php
        $this->logger->debug(esc_html__('The content import step has been displayed', 'omero'));
    }
    ...
    protected function ready() {
        ...
        <?php
        $this->logger->debug(esc_html__('The final step has been displayed', 'omero'));
    }
    ...
    public function generate_child() {
        ...
        if (!file_exists($path)) {
            ...
            $this->logger->debug(esc_html__('The existing child theme was activated', 'omero'));
        ...
        $this->logger->debug(esc_html__('The newly generated child theme was activated', 'omero'));
    ...
    public function _ajax_activate_license() {
        ...
        if (!check_ajax_referer('merlin_nonce', 'wpnonce')) {
            wp_send_json(
                array(
                    'success' => false,
                    'message' => esc_html__('Yikes! The theme activation failed. Please try again or contact support.', 'omero'),
                )
            );
        }
        ...
        if (empty($_POST['license_key'])) {
            wp_send_json(
                array(
                    'success' => false,
                    'message' => esc_html__('Please add your license key before attempting to activate one.', 'omero'),
                )
            );
        }
        ...
        $this->logger->debug(esc_html__('The license activation was performed with the following results', 'omero'), $result);
    ...
    protected function edd_activate_license($license) {
        ...
        if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
            ...
            if (is_wp_error($response)) {
                ...
                $message = esc_html__('An error occurred, please try again.', 'omero');
            }
        } else {
                ...
                switch ($license_data->error) {
                    ...
                    case 'expired':
                        ...
                        /* translators: Expiration date */
                            esc_html__('Your license key expired on %s.', 'omero'),
                            date_i18n(get_option('date_format'), strtotime($license_data->expires, current_time('timestamp')))
                        );
                        break;

                    case 'revoked':
                        $message = esc_html__('Your license key has been disabled.', 'omero');
                        break;

                    case 'missing':
                        $message = esc_html__('This appears to be an invalid license key. Please try again or contact support.', 'omero');
                        break;

                    case 'invalid':
                    case 'site_inactive':
                        $message = esc_html__('Your license is not active for this URL.', 'omero');
                        break;

                    case 'item_name_mismatch':
                        /* translators: EDD Item Name */
                        $message = sprintf(esc_html__('This appears to be an invalid license key for %s.', 'omero'), $this->edd_item_name);
                        break;

                    case 'no_activations_left':
                        $message = esc_html__('Your license key has reached its activation limit.', 'omero');
                        break;
    ...
    public function generate_child_functions_php($slug) {
        ...
        $this->logger->debug(esc_html__('The child theme functions.php content was generated', 'omero'));
    ...
    public function generate_child_style_css($slug, $parent, $author, $version) {
        ...
        $this->logger->debug(esc_html__('The child theme style.css content was generated', 'omero'));
    ...
    public function generate_child_screenshot($path) {
        ...
        if (!empty($screenshot) && file_exists($screenshot)) {
            ...
            $this->logger->debug(esc_html__('The child theme screenshot was copied to the child theme, with the following result', 'omero'), array('copied' => $copied));
        } else {
            $this->logger->debug(esc_html__('The child theme screenshot was not generated, because of these results', 'omero'), array('screenshot' => $screenshot));
        }
    }
    ...
    function _ajax_plugins() {
        ...
        foreach ($plugins['activate'] as $slug => $plugin) {
            if ($_POST['slug'] === $slug) {
                $json = array(
                    ...
                    'action2'       => -1,
                    'message'       => esc_html__('Activating', 'omero'),
                );
                break;
            }
        }
        ...
        foreach ($plugins['update'] as $slug => $plugin) {
            if ($_POST['slug'] === $slug) {
                $json = array(
                    ...
                    'action2'       => -1,
                    'message'       => esc_html__('Updating', 'omero'),
                );
                break;
            }
        }
        ...
        foreach ($plugins['install'] as $slug => $plugin) {
            if ($_POST['slug'] === $slug) {
                $json = array(
                    ...
                    'action2'       => -1,
                    'message'       => esc_html__('Installing', 'omero'),
                );
                break;
            }
        }
        ...
        if ($json) {
            $this->logger->debug(
                esc_html__('A plugin with the following data will be processed', 'omero'),
                array(
            ...
            $json['message'] = esc_html__('Installing', 'omero');
            wp_send_json($json);
        } else {
            $this->logger->debug(
                esc_html__('A plugin with the following data was processed', 'omero'),
                array(
                    'plugin_slug' => sanitize_text_field($_POST['slug']),
                )
            );
            ...
            wp_send_json(
                array(
                    'done'    => 1,
                    'message' => esc_html__('Success', 'omero'),
                )
            );
        }
    ...
    function _ajax_content() {
        ...
        if (!check_ajax_referer('merlin_nonce', 'wpnonce') || empty($_POST['content']) && isset($content[$_POST['content']])) {
            $this->logger->error(esc_html__('The content importer AJAX call failed to start, because of incorrect data', 'omero'));

            wp_send_json_error(
                array(
                    'error'   => 1,
                    'message' => esc_html__('Invalid content!', 'omero'),
                )
            );
        }
        ...
        if (isset($_POST['proceed'])) {
            if (is_callable($this_content['install_callback'])) {
                $this->logger->info(
                    esc_html__('The content import AJAX call will be executed with this import data', 'omero'),
                    array(
        ...
        if ($json) {
            ...
            $this->logger->error(
                esc_html__('The content import AJAX call failed with this passed data', 'omero'),
                array(
            ...
            wp_send_json(
                array(
                    'error'   => 1,
                    'message' => esc_html__('Error', 'omero'),
                    'logs'    => '',
                    'errors'  => '',
                )
            );
    ...
    public function _ajax_get_total_content_import_items() {
        if (!check_ajax_referer('merlin_nonce', 'wpnonce') && empty($_POST['selected_index'])) {
            $this->logger->error(esc_html__('The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'omero'));

            wp_send_json_error(
                array(
                    'error'   => 1,
                    'message' => esc_html__('Invalid data!', 'omero'),
                )
            );
        }
    ...
    protected function get_import_data($selected_import_index = 0) {
        ...
        if (!empty($import_files['content'])) {
            $content['content'] = array(
                'title'            => esc_html__('Content', 'omero'),
                'description'      => esc_html__('Demo content data.', 'omero'),
                'pending'          => esc_html__('Pending', 'omero'),
                'installing'       => esc_html__('Installing', 'omero'),
        ...
        if (!empty($import_files['widgets'])) {
            $content['widgets'] = array(
                'title'            => esc_html__('Widgets', 'omero'),
                'description'      => esc_html__('Sample widgets data.', 'omero'),
                'pending'          => esc_html__('Pending', 'omero'),
                'installing'       => esc_html__('Installing', 'omero'),
                'success'          => esc_html__('Success', 'omero'),
        ...
        if (!empty($import_files['sliders'])) {
            $content['sliders'] = array(
                'title'            => esc_html__('Revolution Slider', 'omero'),
                'description'      => esc_html__('Sample Revolution sliders data.', 'omero'),
                'pending'          => esc_html__('Pending', 'omero'),
                'installing'       => esc_html__('Installing', 'omero'),
                'success'          => esc_html__('Success', 'omero'),
        ...
        if (!empty($import_files['options'])) {
            $content['options'] = array(
                'title'            => esc_html__('Home Page', 'omero'),
                'description'      => esc_html__('Install homepage xml.', 'omero'),
                'pending'          => esc_html__('Pending', 'omero'),
                'installing'       => esc_html__('Installing', 'omero'),
                'success'          => esc_html__('Success', 'omero'),
                'install_callback' => array($this->importer, 'import'),
        ...
        if (!empty($import_files['redux'])) {
            $content['redux'] = array(
                'title'            => esc_html__('Redux Options', 'omero'),
                'description'      => esc_html__('Redux framework options.', 'omero'),
                'pending'          => esc_html__('Pending', 'omero'),
                'installing'       => esc_html__('Installing', 'omero'),
                'success'          => esc_html__('Success', 'omero'),
        ...
        if (false !== has_action('merlin_after_all_import')) {
            $content['after_import'] = array(
                'title'            => esc_html__('After import setup', 'omero'),
                'description'      => esc_html__('After import setup.', 'omero'),
                'pending'          => esc_html__('Pending', 'omero'),
                'installing'       => esc_html__('Installing', 'omero'),
                'success'          => esc_html__('Success', 'omero'),
...
//        $response = $importer->importSliderFromPost(true, true, $file);
        ...
        $this->logger->info(esc_html__('The revolution slider import was executed', 'omero'));
    ...
    public function pt_importer_new_ajax_request_response_data($data) {
        ...
        $data['message']  = esc_html__('Installing', 'omero');
        $data['proceed']  = 'true';
    ...
    public function after_content_import_setup() {
        // Set static homepage.
        $homepage = omero_get_page_by_title(apply_filters('merlin_content_home_page_title', 'Home'));

        if ($homepage) {
            update_option('page_on_front', $homepage->ID);
            update_option('show_on_front', 'page');

            $this->logger->debug(esc_html__('The home page was set', 'omero'), array('homepage_id' => $homepage));
        }

        // Set static blog page.
        $blogpage = omero_get_page_by_title(apply_filters('merlin_content_blog_page_title', 'Blog'));

        if ($blogpage) {
            update_option('page_for_posts', $blogpage->ID);
            update_option('show_on_front', 'page');

            $this->logger->debug(esc_html__('The blog page was set', 'omero'), array('blog_page_id' => $blogpage));
        }
    }
    ...
    public function before_content_import_setup() {
        ...
        $hello_world = omero_get_page_by_title('Hello World!', OBJECT, 'post');

        if (!empty($hello_world)) {
            $hello_world->post_status = 'draft';
            wp_update_post($hello_world);

            $this->logger->debug(esc_html__('The Hello world post status was set to draft', 'omero'));
        }
    }
    ...
    public function validate_import_file_info($import_files) {
        ...
        foreach ($import_files as $import_file) {
            if (!empty($import_file['import_file_name'])) {
                ...
                $this->logger->warning(esc_html__('This predefined demo import does not have the name parameter: import_file_name', 'omero'), $import_file);
            }
        }
    ...
    public function get_import_files_paths($selected_import_index) {
        ...
        if (!empty($selected_import_data['import_rev_slider_file_url'])) {
            ...
            $check_oneclick  = get_option('omero_check_oneclick');
