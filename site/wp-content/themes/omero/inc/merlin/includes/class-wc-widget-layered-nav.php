...
class Omero_Widget_Layered_Nav extends WC_Widget {

    /**
     * Constructor.
     */
    public function __construct() {
        $this->widget_cssclass    = 'woocommerce omero_widget_layered_nav woocommerce-widget-layered-nav';
        $this->widget_description = esc_html__('Display a list of attributes to filter products in your store.(Omero custom)', 'omero');
        $this->widget_id          = 'omero_woocommerce_layered_nav';
        $this->widget_name        = esc_html__('Omero Filter Products by Attribute', 'omero');
        parent::__construct();
    }
    ...
    public function init_settings() {
        ...
        $this->settings = array(
            'title'      => array(
                'type'  => 'text',
                'std'   => esc_html__('Filter by', 'omero'),
                'label' => esc_html__('Title', 'omero'),
            ),
            'attribute'  => array(
                'type'    => 'select',
                'std'     => '',
                'label'   => esc_html__('Attribute', 'omero'),
                'options' => $attribute_array,
            ),
            'query_type' => array(
                'type'    => 'select',
                'std'     => 'and',
                'label'   => esc_html__('Query type', 'omero'),
                'options' => array(
                    'and' => esc_html__('AND', 'omero'),
                    'or'  => esc_html__('OR', 'omero'),
                ),
            ),
        );
    }
    ...
    protected function layered_nav_list($terms, $taxonomy, $query_type) {
        ...
        foreach ($terms as $term) {
            ...
            if ($count > 0 || $option_is_set) {
                ...
                $term_html = '<a rel="nofollow" class="omero-button-type" href="' . esc_url($link) . '">' . esc_html($term->name) . '</a>';
                    ...
                    if (isset($fields[0]["type"])) {
                        ...
                        switch ($fields[0]["type"]) {
                            case 'color':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="omero-color-type" href="' . esc_url($link) . '">';
                                $term_html .= '<span class="color-label-wrapper">';
                                $term_html .= '<span class="color-label" style="background: ' . $color . ';"></span>';
                                $term_html .= '<span class="color-name">' . $term->name . '</span>';
                                $term_html .= '</span>';
                                $term_html .= '<span class="count">(' . $count . ')</span>';
                                $term_html .= '</a>';
                                $class = 'color';
                                break;
                            ...
                            case 'image':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="omero-image-type" href="' . esc_url($link) . '">';
            ...
            echo '<li class="woocommerce-widget-layered-nav-list__item wc-layered-nav-term ' . $class . ($option_is_set ? 'woocommerce-widget-layered-nav-list__item--chosen chosen' : '') . '">';
