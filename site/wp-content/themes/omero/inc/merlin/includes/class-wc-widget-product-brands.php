 ...
 * @extends WC_Widget
 */
class Omero_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'omero');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'omero');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'omero'),
				'label' => esc_html__('Title', 'omero'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'omero'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'omero'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'omero'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'omero'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'omero'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'omero'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'omero'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			require get_theme_file_path('inc/woocommerce/class-product-brand-list-walker.php');

			$list_args['walker']                  = new Omero_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'omero');
			...
			echo '<ul class="product-brands" id="omero-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	omero_add_widget('Omero_Widget_Product_Brands');
});

add_action('init', function () {
	if (version_compare(WC()->version, '9.4.0', '<')) {
		$labels = array(
			'name'                       => esc_html__('Brands', 'omero'),
			'singular_name'              => esc_html__('Brands', 'omero'),
			'menu_name'                  => esc_html__('Brands', 'omero'),
			'all_items'                  => esc_html__('All Brands', 'omero'),
			'parent_item'                => esc_html__('Parent Brand', 'omero'),
			'parent_item_colon'          => esc_html__('Parent Brand:', 'omero'),
			'new_item_name'              => esc_html__('New Brand Name', 'omero'),
			'add_new_item'               => esc_html__('Add New Brands', 'omero'),
			'edit_item'                  => esc_html__('Edit Brand', 'omero'),
			'update_item'                => esc_html__('Update Brand', 'omero'),
			'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'omero'),
			'search_items'               => esc_html__('Search Brands', 'omero'),
			'add_or_remove_items'        => esc_html__('Add or remove Brands', 'omero'),
			'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'omero'),
		);
		$args   = array(
			'labels'            => $labels,
			'hierarchical'      => true,
		...
		omero_add_tx('product_brand', 'product', $args);
	}
});
