...
class Omero_Woocommerce_Extra {
    ...
    public function add_deal_fields() {
        ...
        <p class="form-field _deal_quantity_field">
            <label for="_sale_quantity"><?php esc_html_e('Sale quantity', 'omero') ?></label>
            <?php echo wc_help_tip(__('Set this quantity will make the product to be a deal. The sale will end when this quantity is sold out.', 'omero')); ?>
            ...
            <?php
            if ($quantity > 0) { ?>
                <span class="deal-sold-counts"><strong> <?php echo wp_kses_data(sprintf(_n('%s product is sold', '%s products are sold', $abc, 'omero'), $sales_counts)) ?></strong></span>
            <?php }
            ?>
        </p>
    ...
    public function quantity_input_args($args, $product) {
        if (!omero_woocommerce_is_deal_product($product)) {
            return $args;
        }
